...
existing archives, and egg-info metadata. The manifest reports release artifacts such
as `dist/`, `build/`, `optimizer_results/`, `.egg-info`, and nested source archives as
hygiene failures while tolerating transient interpreter caches generated by local test
runs because those files are never included in the deterministic archive.
