...
def windows(start: int, end: int, count: int, train_ratio: float, anchor_mode: str = 'rolling') -> list[dict[str, tuple[int, int]]]:
...
def ranged_runner(runner: Any, range_: tuple[int, int], tag: str) -> Any:
    ...
    if caps and getattr(caps, 'supports_runner_request', False) and getattr(caps, 'supports_range', False):
        def call(params: dict[str, object]) -> Any:
            return runner(RunnerRequest(params=params, trial_id=0, required_metrics=set(), required_outputs=set(), early_stop_conditions=[], range=range_, tags={'walk_forward': tag}))
        return call
    if hasattr(runner, 'with_range'):
        return runner.with_range(*range_)
    raise ValueError('walk-forward requires runner.capabilities.supports_range or with_range(start,end)')
...
def run(parameters: Any, runner: Any, base_config: OptimizerConfig, *, start: int, end: int):
    ...
    results = []
    diags = []
    for idx, w in enumerate(windows(start, end, base_config.walk_forward_windows, base_config.walk_forward_train_ratio, base_config.walk_forward_anchor_mode), start=1):
        ...
        if best is not None:
            ...
            test_res = optimize([], ranged_runner(runner, w['test'], 'test'), test_cfg) if False else None
