...
def analyze_trial(raw: Any, simulations: int = 1000, seed: int = 42) -> dict[str, object]:
    ...
    if not profits:
        return {'status': 'insufficient_data', 'simulations': 0, 'p05_net_profit': None, 'median_net_profit': None, 'probability_positive': None}
    ...
    return {'status': 'ok', 'simulations': len(totals), 'p05_net_profit': q(0.05), 'median_net_profit': q(0.5), 'mean_net_profit': mean(totals), 'probability_positive': sum(1 for x in totals if x > 0) / len(totals)}
...
def analyze(trials: list[Any], simulations: int = 1000, seed: int = 42) -> dict[str, object]:
    ...
    return {'status': 'ok' if ok else 'insufficient_data', 'method': 'bootstrap_trades', 'by_trial_id': per, 'diagnostics': [] if ok else [{'code': 'MONTE_CARLO_REQUIRES_TRADES', 'severity': 'warning', 'message': 'Expected saved backtest_result trades'}]}
