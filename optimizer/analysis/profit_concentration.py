...
def analyze_trial(raw: Any) -> dict[str, object]:
    ...
    if not positive or total <= 0:
        return {'status': 'insufficient_data', 'trade_count': len(profits), 'top_10_percent_share': None, 'score': None}
    n = max(1, int(len(positive) * 0.1 + 0.999999))
    share = sum(positive[:n]) / total
    return {'status': 'ok', 'trade_count': len(profits), 'top_10_percent_share': share, 'score': max(0.0, min(1.0, 1.0 - share))}
...
def analyze(trials: list[Any]) -> dict[str, object]:
    ...
    for t in trials:
        ...
        if item['status'] != 'ok' and t.metrics.get('profit_concentration_score') is not None:
            item = {'status': 'ok', 'trade_count': None, 'top_10_percent_share': None, 'score': t.metrics.get('profit_concentration_score')}
        per[t.id] = item
    ok = [v['score'] for v in per.values() if v.get('score') is not None]
    return {'status': 'ok' if ok else 'insufficient_data', 'by_trial_id': per, 'diagnostics': [] if ok else [{'code': 'PROFIT_CONCENTRATION_REQUIRES_TRADES', 'severity': 'warning', 'message': 'Expected closed_trades/trades with profit or pnl'}]}
