...
def load_obj(spec):
    ...
    modname = "optimizer_user_" + Path(file).stem
    sp = importlib.util.spec_from_file_location(modname, file)
...
def _cmd_export(result_dir, fmt):
    ...
    if fmt == "json":
        ...
        for r in rows[:50]:
            lines.append(
                f"- id={r.get('id')} status={r.get('status')} objective={r.get('objective_value')} params={r.get('params')}"
            )
        out.write_text("\n".join(lines) + "\n")
    print(out)
...
def _cmd_analyze(result_dir):
    ...
    best = sorted(
        [r for r in rows if r.get("objective_value") is not None],
...
def main(argv=None):
    ...
    exp.add_argument("--result-dir", default="./optimizer_results")
    exp.add_argument("--format", choices=["json", "csv", "markdown"], default="json")
    ...
    for name in ["walk-forward", "champions", "diff", "plot", "baseline"]:
        ...
        p.add_argument("--result-dir", default="./optimizer_results")
    ...
    if ns.cmd in {"run", "resume"}:
        ...
        to_json(res, Path(ns.output_dir) / "result.json")
        ...
        print(
            f"{ns.cmd}: no standalone CLI implementation in this release; use the documented Python API/reporting module for this operation.",
            file=sys.stderr,
        )
        return 2
