...
def _apply_bin(op: ast.operator, a: Any, b: Any) -> Any:
    ...
    if isinstance(op, ast.Pow):
        ...
        return _finite(a ** b)
    if isinstance(op, ast.Add): return _finite(a + b)
    if isinstance(op, ast.Sub): return _finite(a - b)
    if isinstance(op, ast.Mult): return _finite(a * b)
    if isinstance(op, ast.Div): return _finite(a / b)
    if isinstance(op, ast.Mod): return _finite(a % b)
    raise SafeExpressionError(f'unsafe expression op {type(op).__name__}')


def safe_eval(expr: str, names: dict[str, Any], *, mode: Literal['numeric', 'boolean', 'any'] = 'any') -> Any:
    def ev(n):
        if isinstance(n, ast.Expression): return ev(n.body)
        if isinstance(n, ast.Constant): return _finite(n.value)
        if isinstance(n, ast.Name):
            if n.id in names: return _finite(names[n.id])
        ...
        if isinstance(n, ast.UnaryOp) and isinstance(n.op, (ast.Not, ast.USub, ast.UAdd)):
            v = ev(n.operand)
            return (not bool(v)) if isinstance(n.op, ast.Not) else _finite(-v if isinstance(n.op, ast.USub) else +v)
        ...
        if isinstance(n, ast.Compare):
            ...
            for op, comp in zip(n.ops, n.comparators):
                ...
                else: raise SafeExpressionError(f'unsafe comparator {type(op).__name__}')
