...
class MetricRegistry:
    METRICS = {
        ...
        "sharpe_ratio": MetricSpec(
            "sharpe_ratio", "runner", "maximize", required_outputs={"summary_metrics"}
        ),
        "sortino_ratio": MetricSpec(
            "sortino_ratio", "runner", "maximize", required_outputs={"summary_metrics"}
        ),
        ...
        "return_drawdown_ratio": MetricSpec(
            ...
            "optimizer_derived",
            "maximize",
            required_metrics={"net_profit", "max_drawdown"},
            required_outputs={"summary_metrics"},
        ),
        "expectancy": MetricSpec(
            "expectancy", "runner", "maximize", required_outputs={"summary_metrics"}
        ),
        "robustness_score": MetricSpec(
            "robustness_score",
            "optimizer_analysis",
            "maximize",
            requires_completed_trials=True,
            can_be_objective=False,
        ),
        "overfitting_score": MetricSpec(
            "overfitting_score",
            "optimizer_analysis",
            "minimize",
            requires_completed_trials=True,
        ...
        "profit_concentration_score": MetricSpec(
            "profit_concentration_score",
            "optimizer_analysis",
            "minimize",
            required_outputs={"closed_trades"},
            requires_trade_level_data=True,
            requires_completed_trials=True,
            can_be_objective=False,
        ),
    }
    ...
    def get(self, name):
        return self.METRICS.get(name, MetricSpec(name, "runner", "neutral"))
    ...
    def extract_expression_metrics(self, expression: str | None) -> set[str]:
        ...
        visitor.visit(ast.parse(expression, mode="eval"))
    ...
    def get_required_metrics(self, names) -> set[str]:
        ...
        while changed:
            ...
            for n in list(req):
                before = len(req)
                req |= self.get(n).required_metrics
                changed |= len(req) != before
        return req

    def required_metrics(self, names):
        return self.get_required_metrics(names)

    def get_required_runner_metrics(self, names) -> set[str]:
        return {
            n
            for n in self.get_required_metrics(names)
            if self.get(n).source in {"runner", "external"}
        }

    def get_required_outputs(self, names) -> set[str]:
        outs = set()
        for n in self.get_required_metrics(names):
            outs |= self.get(n).required_outputs
        return outs

    def required_outputs(self, names):
        return self.get_required_outputs(names)
    ...
    def profile_required_metrics(self, profiles) -> set[str]:
        ...
        for p in profiles or []:
            req |= self.PROFILE_METRICS.get(p, set())
