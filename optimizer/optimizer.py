...
def _trial_from_raw(d):
    ...
    payload['diagnostics'] = [x if isinstance(x, Diagnostic) else Diagnostic(**{k: v for k, v in x.items() if k in diag_fields}) for x in payload.get('diagnostics', [])]
...
def _check_parallel_policy(config, diagnostics):
    ...
    if config.max_parallel > cpu:
        ...
        if config.max_parallel_over_cpu_policy == 'warn':
            diagnostics.append(Diagnostic('MAX_PARALLEL_OVER_CPU', msg, 'warning', context={'max_parallel': config.max_parallel, 'cpu_count': cpu}))


def _run_jobs(jobs, runner, config, space_hash, config_hash, store):
    trials = []
    if config.max_parallel > 1 and len(jobs) > 1:
        executor_cls = ProcessPoolExecutor if config.parallel_backend == 'process' else ThreadPoolExecutor
...
def _skipped_trial(tid, params, config, space_hash, config_hash, message):
    ...
    return Trial(tid, dict(params), {}, None, direction, None, False, {'parameters': message}, 1, None, None, None, None, None, 0.0, 'skipped', parameter_space_hash=space_hash, optimizer_config_hash=config_hash, diagnostics=[Diagnostic('INVALID_PARAM_COMBINATION', message, 'warning', tid, ph, context={'params': dict(params)})], params_hash=ph)
...
def optimize(
    ...
    *,
    ...
    done_hashes = {x.get('params_hash') or stable_hash(x.get('params', {})) for x in raw_existing if x.get('status') == 'completed'}
    trials = list(existing_trials)
    next_id = max([t.id for t in trials] or [0]) + 1
    if config.baseline_params and config.run_baseline_first and stable_hash(config.baseline_params) not in done_hashes:
    ...
    if base_cmp.get('recommended_worse_than_baseline'):
        diagnostics.append(Diagnostic('RECOMMENDED_WORSE_THAN_BASELINE', 'Recommended trial is worse than configured baseline', 'warning', context=base_cmp))
    diagnostics.append(Diagnostic('ADVANCED_FEATURES_ACTIVE', 'Advanced algorithms/analysis run natively when enabled; optional reports degrade with diagnostics if dependencies/data are missing', 'info'))
    if not [t for t in trials if t.status == 'completed' and t.passed_constraints]:
        ...
        diagnostics.append(Diagnostic('NO_TRIALS_PASSED_CONSTRAINTS', 'No completed trials passed hard constraints', 'warning', context={'top_rejection_causes': sorted(causes.items(), key=lambda x: x[1], reverse=True)[:5]}))
    res = OptimizerResult(rec, rec_name, profiles['best_objective'].trial, ranked[: config.top_n], trials if config.save_all_trials else None, [t for t in trials if t.passed_constraints], [t for t in trials if t.status == 'completed' and not t.passed_constraints], str(getattr(store, 'path', config.output_dir)), counts, profiles, profiles['best_objective'].trial, profiles['best_passed_constraints'].trial, profiles['best_balanced'].trial, profiles['most_robust'].trial, profiles['best_profit'].trial, profiles['best_drawdown'].trial, profiles['best_profit_factor'].trial, profiles['best_sharpe'].trial, pf, diagnostics=diagnostics, analysis=_analysis(config, trials, space), baseline_trial=baseline_trial, baseline_comparison=base_cmp)
    return res
