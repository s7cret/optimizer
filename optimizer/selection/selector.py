...
def build_profiles(trials, config):
    ...
    bp = _best(passed, lambda t: objective_sort_value(t.objective_value, t.objective_direction), True)
    profiles['best_passed_constraints'] = ResultProfile('best_passed_constraints', bp, 'best objective among trials passing hard thresholds', 'objective_value', None if bp is None else bp.objective_value)
    ...
    for name, metric, reason in [('best_profit', 'net_profit', 'highest net profit'), ('best_drawdown', 'max_drawdown_percent', 'best drawdown by registry direction'), ('best_profit_factor', 'profit_factor', 'highest profit factor'), ('best_sharpe', 'sharpe_ratio', 'highest Sharpe ratio')]:
