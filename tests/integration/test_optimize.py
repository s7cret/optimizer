...
def test_public_optimize_routes_grid_random_adaptive(tmp_path):
    ...
    for name, extra in cases:
        cfg = OptimizerConfig(algorithm=name, output_dir=tmp_path / name, storage_backend='json', **extra)
...
def test_public_optimize_walk_forward_requires_range_capable_runner(tmp_path):
    ...
    with pytest.raises(ValueError, match=r'requires runner\.capabilities\.supports_range|with_range'):
