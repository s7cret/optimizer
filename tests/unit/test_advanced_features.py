...
def runner(p):
    x = p['x']; y = p['y']
    return {'net_profit': -(x - 5) ** 2 + y, 'max_drawdown_percent': 10 - y, 'profit_factor': 1 + y / 10, 'sharpe_ratio': x / 10}


def test_genetic_optimizer_runs_real_generations(tmp_path):
    cfg = OptimizerConfig(algorithm='genetic', output_dir=tmp_path, storage_backend='json', max_trials=10, genetic_population_size=4, genetic_generations=4, seed=7)
...
def test_bayesian_optimizer_proposes_after_warmup(tmp_path):
    cfg = OptimizerConfig(algorithm='bayesian', output_dir=tmp_path, storage_backend='json', max_trials=8, bayesian_trials=8, bayesian_warmup_random_trials=3, seed=3)
    res = optimize(params(), runner, cfg)
...
def test_analysis_features_use_saved_trade_and_train_test_data(tmp_path):
    def trade_runner(p):
        return {'net_profit': p['x'], 'train_net_profit': p['x'] * 2, 'test_net_profit': p['x'], 'max_drawdown_percent': 1, 'closed_trades': [{'profit': 10}, {'profit': 1}, {'profit': -2}]}
    cfg = OptimizerConfig(output_dir=tmp_path, storage_backend='json', max_trials=3, save_backtest_result=True, analysis_profile='full', robustness_min_neighbors=1, monte_carlo_simulations=20)
...
def test_diff_and_plot_html(tmp_path):
    res_a = optimize([Parameter('x', 'int', 1, 1, 1, 1)], lambda p: {'net_profit': 1, 'max_drawdown_percent': 1}, OptimizerConfig(output_dir=tmp_path/'a', storage_backend='json'))
    res_b = optimize([Parameter('x', 'int', 2, 2, 2, 1)], lambda p: {'net_profit': 2, 'max_drawdown_percent': 1}, OptimizerConfig(output_dir=tmp_path/'b', storage_backend='json'))
...
def test_walk_forward_range_runner(tmp_path):
    ...
    cfg = OptimizerConfig(algorithm='grid', output_dir=tmp_path, storage_backend='json', max_trials=2, walk_forward_windows=2)
