...
from optimizer import OptimizerConfig, Parameter, ParameterSpace, optimize
...
def test_diagnostic_public_signature_order():
    ...
    assert d.to_dict() == {'code': 'C', 'message': 'msg', 'severity': 'error', 'trial_id': 7, 'params_hash': 'hash', 'metric': 'metric', 'context': {'x': 1}}


def test_minimize_objective_ranking_and_profiles(tmp_path):
    def runner(p):
        return {'max_drawdown_percent': p['x'], 'net_profit': 10 - p['x'], 'profit_factor': 1.1, 'sharpe_ratio': 1}
    cfg = OptimizerConfig(output_dir=tmp_path, storage_backend='json', objective='max_drawdown_percent', objective_direction='auto', max_trials=3, use_profile_auto_constraints=False)
...
def test_resume_uses_params_hash_and_loads_prior_trials(tmp_path):
    ...
    def runner(p):
        calls.append(p['x'])
        return {'net_profit': p['x'], 'max_drawdown_percent': 1, 'profit_factor': 1.1, 'sharpe_ratio': 1}
    params = [Parameter('x', 'int', 1, 1, 2, 1)]
    cfg = OptimizerConfig(output_dir=tmp_path, storage_backend='json', max_trials=2, use_profile_auto_constraints=False)
    first = optimize(params, runner, cfg)
...
def test_invalid_grid_combos_are_saved_as_skipped(tmp_path):
    res = optimize(
        ...
        lambda p: {'net_profit': p['x'] + p['y'], 'max_drawdown_percent': 1, 'profit_factor': 1.1, 'sharpe_ratio': 1},
        OptimizerConfig(output_dir=tmp_path, storage_backend='json', max_trials=4, use_profile_auto_constraints=False),
...
def test_metric_registry_expression_and_profile_requirements():
    reg = MetricRegistry()
    assert reg.extract_expression_metrics('net_profit / max_drawdown_percent') == {'net_profit', 'max_drawdown_percent'}
...
def test_timeout_returns_timeout_status_without_waiting_for_runner_completion(tmp_path):
    ...
    res = optimize([Parameter('x', 'int', 1, 1, 1, 1)], slow, OptimizerConfig(output_dir=tmp_path, storage_backend='json', timeout_per_trial_sec=0.05, use_profile_auto_constraints=False))
...
def test_baseline_comparison_warns_when_recommendation_worse(tmp_path):
    cfg = OptimizerConfig(output_dir=tmp_path, storage_backend='json', baseline_params={'x': 5}, max_trials=1, use_profile_auto_constraints=False)
    res = optimize([Parameter('x', 'int', 1, 1, 1, 1)], lambda p: {'net_profit': p['x'], 'max_drawdown_percent': 1, 'profit_factor': 1.1, 'sharpe_ratio': 1}, cfg)
